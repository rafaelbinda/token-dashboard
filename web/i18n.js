...
const DICT = {
  ...
  pt: {
    ...
    session_dt:'Sessão', session_dd:'Uma execução do Claude Code (do início ao fim). Cada sessão é um arquivo <code>.jsonl</code>.',
    turn_dt:'Turno', turn_dd:'Uma mensagem que você enviou ao Claude. Cada turno gera uma resposta (possivelmente com chamadas de ferramentas).',
    input_dt:'Tokens de entrada', input_dd:'O novo texto que você (e resultados de ferramentas) enviou ao Claude neste turno. Cobrado pela taxa integral de entrada.',
    output_dt:'Tokens de saída', output_dd:'O texto que o Claude escreveu de volta. Cobrado pela maior taxa — geralmente o maior custo por turno.',
    cread_dt:'Cache lido', cread_dd:'Tokens que o Claude reutilizou do cache (seu CLAUDE.md, arquivos anteriores, histórico). ~10× mais barato que entrada normal. Contagens altas = boa prática de custo.',
    ccreate_dt:'Cache criado', ccreate_dd:'Gravar algo no cache pela primeira vez. Custo único; compensa no próximo turno.',
    billable_dt:'Tokens faturáveis', billable_dd:'Entrada + Saída + Cache criado. Leituras de cache são cobradas separadamente (e muito mais barato).',
